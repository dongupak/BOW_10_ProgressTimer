...
-(id) init
{
	if( (self=[super init])) {
        ...
        // ptBullet의 총알 이미지가 줄어드는 형태를 지정해주는 옵션입니다.
        //ccp(1,0)으로 지정하면 가로로 이미지가 줄어들고 ccp(0,1)로 지정하면 세로로 이미지가 줄어듭니다.
        ptBullet.barChangeRate = ccp(1,0);
        //ptBullet.barChangeRate = ccp(0,1);
        // ptBullet의 총알 이미지가 줄어드는 방향(왼쪽 또는 오른쪽)을 지정해주는 옵션입니다.
        // ccp(0,1)이면 오른쪽부터 이미지가 줄어들고 ccp(1,0)이면 왼쪽부터 이미지가 줄어듭니다.
        //ptBullet.midpoint = ccp(1,0);
        ptBullet.midpoint = ccp(0,1);
        // ptBullet의 초기 비율을 100으로 잡습니다.
        ptBullet.percentage=50;
...
-(void)ccTouchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    //총 총알 개수 7에서 터치가 될 때마다 -1씩한다.
    bulletCount -= 1;
    //CCProgressTimer의 속성인 .percent가 처음엔 100 총알 개수가 -1될 때마다 그 비율을 계산해서 이미지를 줄여 준다.
    ptBullet.percentage = bulletCount *100 / MAX_BULLET_COUNT;
    NSLog(@"bulletCount = %i, ptBullet.percent = %5.2f", bulletCount,
          ptBullet.percentage);
}
